...
| Auth | Supabase | `AUTH_PROVIDER=supabase` (EE `SupabaseAuthStrategy`) |
| App + EE database | Supabase Postgres | `DATABASE_DIALECT=postgres`, `DATABASE_URL` (migrations run at boot) |
...
3. Auth → enable the providers you want (email at minimum). The EE frontend
...
2. **Pages** (×3): connect the GitHub repos (native GitOps — see below).
   - App: build `npm ci && npm run build`, output `dist`, env
     `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY` (whatever the EE register
     module reads).
...
- [ ] `AUTH_PROVIDER=supabase` and the EE tree present in the image build
      (`knowledge-loom-ee/scripts/link-dev.sh` or a merge step in CI).
