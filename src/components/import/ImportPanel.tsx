/**
 * ImportPanel — the reusable "bring outside material in" form.
 *
 * Accepts a PDF, plain-text/markdown file, audio/video recording, photo of
 * handwritten notes, or pasted text. POST /api/import extracts the text
 * (transcribing audio when configured), queues an AI job that structures it
 * into a note, and this panel polls the job until the note lands, then offers
 * to open it.
 *
 * Rendered both as a tab inside the Capture box (Home) and as the standalone
 * /import page, so the logic lives here and the two callers just wrap it.
 */
