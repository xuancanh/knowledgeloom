...
billing/org/admin features live in a **private enterprise repo**
(`knowledge-loom-ee`) that is merged into this tree at build time. OSS code
never imports `ee/` — enforced by ESLint; enterprise modules attach through
two seams: the frontend EE registry (`src/lib/ee.ts`) and the backend
