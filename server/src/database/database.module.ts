...
const learnProgressTableProvider = {
  provide: LEARN_PROGRESS_TABLE,
  inject: [ConfigService],
  useFactory: (config: ConfigService) => {
    const dialect = config.get<string>('databaseDialect') || 'sqlite';
    return dialect === 'postgres' ? schema.pgLearnProgress : schema.sqliteLearnProgress;
  },
};
...
const sharesTableProvider = {
  provide: SHARES_TABLE,
  inject: [ConfigService],
  useFactory: (config: ConfigService) => {
    const dialect = config.get<string>('databaseDialect') || 'sqlite';
    return dialect === 'postgres' ? schema.pgShares : schema.sqliteShares;
  },
};

const shareAccessesTableProvider = {
  provide: SHARE_ACCESSES_TABLE,
  inject: [ConfigService],
  useFactory: (config: ConfigService) => {
    const dialect = config.get<string>('databaseDialect') || 'sqlite';
    return dialect === 'postgres' ? schema.pgShareAccesses : schema.sqliteShareAccesses;
  },
};

const marketplaceListingsTableProvider = {
  provide: MARKETPLACE_LISTINGS_TABLE,
  inject: [ConfigService],
  useFactory: (config: ConfigService) => {
    const dialect = config.get<string>('databaseDialect') || 'sqlite';
    return dialect === 'postgres' ? schema.pgMarketplaceListings : schema.sqliteMarketplaceListings;
  },
};

const marketplaceRatingsTableProvider = {
  provide: MARKETPLACE_RATINGS_TABLE,
  inject: [ConfigService],
  useFactory: (config: ConfigService) => {
    const dialect = config.get<string>('databaseDialect') || 'sqlite';
    return dialect === 'postgres' ? schema.pgMarketplaceRatings : schema.sqliteMarketplaceRatings;
  },
};
