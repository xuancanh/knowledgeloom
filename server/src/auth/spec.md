...
See `docs/OPEN_SOURCE_DECISION.md` for the OSS/EE split this design serves.
...
- **SupabaseAuthStrategy** (enterprise — `server/src/ee/auth/`, private repo)
...
| Condition | Strategy |
|---|---|
| `AUTH_PROVIDER=supabase` or `SUPABASE_JWT_SECRET` set | SupabaseAuthStrategy (dynamic import from `ee/`) |
| otherwise | LocalAuthStrategy |

If an EE provider is requested but `ee/` is not present in the build, **boot
...
|---------|---------|-------------|
| `AUTH_PROVIDER` | `local` | `local` or `supabase` (EE) |
| `AUTH_SECRET` | — | Optional bearer token for the local provider |
| `SUPABASE_JWT_SECRET` | — | HS256 secret (EE; implies `supabase` provider) |
...
- GIVEN `AUTH_SECRET` is not set (and no EE provider is selected)
...
### Feature: EE provider selection

**Scenario: Supabase requested without ee/**
- GIVEN `AUTH_PROVIDER=supabase` (or `SUPABASE_JWT_SECRET` set) in an OSS build
- WHEN the server boots
- THEN boot fails with an error naming the missing enterprise modules

### Feature: JWT authentication (EE — SupabaseAuthStrategy)

**Scenario: Valid Bearer token**
- GIVEN `SUPABASE_JWT_SECRET` is set and the EE modules are present
