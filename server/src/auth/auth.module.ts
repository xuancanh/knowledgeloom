 ...
 *   - 'supabase'         — SupabaseAuthStrategy from the enterprise ee/
 *                          modules. Also selected implicitly when
 *                          SUPABASE_JWT_SECRET is configured, for backward
 *                          compatibility with existing deployments.
 *
 * The Supabase strategy lives in server/src/ee/ (merged from the private EE
 * repo at build time — see docs/OPEN_SOURCE_DECISION.md) and is loaded via a
 * dynamic import so the OSS build has no static reference to EE code. If an
 * EE provider is requested but ee/ is absent, boot fails loudly rather than
...
const authStrategyProvider = {
  ...
  useFactory: async (config: ConfigService) => {
    ...
    if (requested === 'supabase') {
      ...
      // exists in enterprise builds where ee/ has been merged into the tree.
      const eeStrategyPath = '../ee/auth/supabase-auth.strategy';
      try {
        ...
        throw new Error(
          'AUTH_PROVIDER=supabase (or SUPABASE_JWT_SECRET) requires the enterprise ee/ modules, '
